...
int main(int argc, char *argv[]) 
{
    ...
    print_matrix(**X_train, rows_train, cols_train);
...
void read_matrix(char *filename, double ***X, double **Y, int *rows, int *cols) 
{
    ...
    *X = (double **)malloc(*rows * sizeof(double *));           // Allocate and read X matrix
    ...
    *Y = (double *)malloc(*rows * sizeof(double));              // Allocate and read Y matrix
    for (int i = 0; i < *rows; i++) 
    {
        fscanf(file, "%lf", &(*Y)[i]);
    }
